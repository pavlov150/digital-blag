...
(function($){
	...
	$.fn.contactable = function(options) {
		...
		var defaults = {
			...
			recievedMsg : 'Ваше письмо отправлено. В ближайшее время мы вам ответим',
  ...
  success: function(data){
						...
						if( data == 'success') {
							...
							if(options.hideOnSubmit == true) {
								...
								$(this_id_prefix+'#contactForm').animate({dummy:1}, 5000).animate({"marginLeft": "-=450px"}, "slow");
								$(this_id_prefix+'div#contactable_inner').animate({dummy:1}, 5000).animate({"marginLeft": "-=447px"}, "slow").animate({"marginLeft": "+=5px"}, "fast"); 
								$(this_id_prefix+'#overlay').css({display: 'none'});	
							}
						} else {
